...
public class ZuoraObjectSchema {
  ...
  boolean hasListFields() {
    return fields.stream()
        .anyMatch(x -> x.getType().getType().equals(SchemaFieldType.ARRAY));
  }
  ...
  private void generateAsMapMethod(StringBuilder builder, List<ZuoraObjectField> fields) {
    ...
    fields.stream()
        ...
        .forEach(x -> {
          appendLine(builder, "    addCustomField(\"%s\", %s, %s.class);",
  ...
  public String toString() {
    ...
    if (fields != null) {
      fields.stream()
          .filter(x -> !x.getCustom())
          .forEach(x -> appendLine(builder, x.toString()));
      appendLine(builder, "");
